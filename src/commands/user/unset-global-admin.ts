...
export default class UnsetGlobalAdmin extends KysoCommand {
  static description = 'Unset user as global admin';

  static examples = [`How to use: \n\t$ kyso user unset-global-admin <email@kyso.io>`];

  static args = [
    {
      name: 'email',
      description: 'User mail to unset as a global admin: email@kyso.io',
      required: true,
    },
  ];

  private async unsetUserAsGlobalAdmin(api: Api, email: string): Promise<void> {
    ...
    try {
      await api.removeUserAsGlobalAdmin(userId);
      this.log(`User with email '${email}' is no longer a global admin`);
    } catch (e: any) {
      // const errorResponse: ErrorResponse = e.response.data;
  ...
  async run(): Promise<void> {
    const { args } = await this.parse(UnsetGlobalAdmin);
    ...
    if (!tokenPermissions.global || !Helper.isGlobalAdmin(tokenPermissions)) {
      this.error("You don't have permissions unset users as global admins");
    }
    await this.unsetUserAsGlobalAdmin(api, args.email);
  }
}
